...
<html>
<head>
<title>忘记密码 - ${CacheUtil.getConfig('SHOP_NAME')}</title>
...
$(function($){
	$('#submit').click(function(){
		...
		if(username=='') {strErr.push("请输入用户名！\n");}
		var email = $.trim($('#email').val());
		if(email=='') {strErr.push("您注册时填写的电子邮箱！\n");}
		...
		$.ajax({
			...
			success: function (json) {
				if (json.status=="success"){
					$('#J_tips').html("密码重置链接已发送至邮箱").show().delay(2000).fadeOut(1000);
				}else{
					alert(json.message);
				}
			}
...
<body>
<c:set var="header_title" value="忘记密码" scope="request" />
            ...
            <form id="frm_login">
               <input type="hidden" id="returnurl" value="${returnurl}">
                ...
                <span class="new-input-span mg-b15">
                    <input type="text" class="new-input" name="username" id="username" value="" placeholder="请输入用户名">
                </span>
                <span class="new-input-span mg-b15">
                    <input type="text" class="new-input" name="email" id="email" value="" placeholder="您注册时留下的电子邮箱">
                </span>
                <a href="javascript:void(0);" id="submit" class="new-abtn-type new-mg-t15">验证信息</a>
            </form>
        </div>
    </div>
</section>
