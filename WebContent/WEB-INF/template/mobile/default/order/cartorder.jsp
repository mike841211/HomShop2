		...
		$.ajax({
			...
			success: function (json) {
				if (json.status=="success"){
					...
					$("#J_totalPrice").val(json.result.totalPrice);
...
<section>
	...
	<div class="tbl mg-t10">
		...
		<div class="tbl-cell cell4">￥<span class="red2" id="J_total_price">0.00</span> 元
		<input type="hidden" name="totalPrice" id="J_totalPrice" value="0"></div>
	</div>
	<div class="tbl">
...
<section>
	<a href="javascript:void(0)" id="submit" class="new-abtn-type">提交订单</a><input type="hidden" name="token" value="${token}" />
</section>
<section class="details">
	<div class="caption">商品明细</div>
	<div class="search-lst">
		<ul class="product-lst">
			<c:set value="0" var="totalWeight"/>
			...
			<c:set value="${totalWeight+sku.weight*cartItem.quantity}" var="totalWeight"/>
            <li class="product-li">
                <a href="${base}/product/item/${product.id}.htm" class="product-a">
                    <span class="product-img"><img src="${empty sku.sampleImage ? empty product.sampleImage ? noimage : product.sampleImage : sku.sampleImage}" alt="${product.name}"></span>
                    <span class="product-info">
                        <strong class="l2h product-name">${product.name}</strong>
						<span class="l2h"><span class="sn">${sku.sn}</span> <span class="red2">${sku.specificationValueName}</span></span>
                        <span><strong class="red2">￥${sku.price}</strong> x ${cartItem.quantity}<c:if test="${discount<1}"> x ${discount}(会员折扣)</c:if></span>
					</span>
                </a>
            </li>
			</c:forEach>
